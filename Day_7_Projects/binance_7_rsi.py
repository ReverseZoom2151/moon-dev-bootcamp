...
symbol = 'BTCUSDT'
size = 1
default_params = {'timeInForce': 'GTX'}
timeframe = '15m'
limit = 100
sma = 20
target = 9
...
def kill_switch(symbol=symbol):
    ...
    try:
        ...
        if pos[1]:
            ...
            if side == 'sell':
                binance.create_limit_sell_order(symbol, kill_size, ask, default_params)
            else:
                binance.create_limit_buy_order(symbol, kill_size, bid, default_params)
            time.sleep(30)
