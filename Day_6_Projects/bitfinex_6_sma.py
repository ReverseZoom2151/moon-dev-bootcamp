...
# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler('bitfinex_trading_bot.log', encoding='utf-8'), logging.StreamHandler()])
logger = logging.getLogger()

# Configuration
CONFIG = {'default_symbol': 'BTC:UST', 'default_size': 0.0001, 'timeframe': '15m', 'limit': 100, 'sma_period': 20, 'target_profit_percentage': 9, 'max_loss_percentage': -8, 'max_risk_amount': 1000, 'order_params': {'timeInForce': 'POC'}, 'sleep_time': 30, 'emergency_timeout': 300}
...
def test_api_connection():
    if demo_mode:
        return True
    try:
        bitfinex.fetch_ticker(CONFIG['default_symbol'])
...
def get_open_positions(symbol=CONFIG['default_symbol']):
    if demo_mode:
        return random.choice([([{'side': 'long', 'amount': 0.0001}], True, 0.0001, True), ([], False, 0, None)])
    try:
        positions = bitfinex.fetch_positions([symbol])
        if not positions or positions[0]['amount'] == 0:
            return [], False, 0, None
        pos = positions[0]
        return positions, True, pos['amount'], pos['side'] == 'long'
    except Exception as e:
...
def calculate_sma(symbol=CONFIG['default_symbol'], timeframe=CONFIG['timeframe'], limit=CONFIG['limit'], sma_period=CONFIG['sma_period']):
    ...
    try:
        bars = bitfinex.fetch_ohlcv(symbol, timeframe, limit=limit)
        df = pd.DataFrame(bars, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        df['sma'] = df['close'].rolling(sma_period).mean()
        current = bitfinex.fetch_ticker(symbol)['last']
...
def execute_trade(symbol, signal, size=CONFIG['default_size']):
    ...
    try:
        current = bitfinex.fetch_ticker(symbol)['last']
        if signal == 'BUY':
            bitfinex.create_limit_buy_order(symbol, size, current)
        elif signal == 'SELL':
            bitfinex.create_limit_sell_order(symbol, size, current)
    except Exception as e:
