...
from pycoingecko import CoinGeckoAPI # Added for CoinGecko API

# --- Configuration ---
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# --- Data Source Configuration ---
# Choose data source type: 'file' or 'coingecko'
DATA_SOURCE_TYPE = 'coingecko'  # Options: 'file', 'coingecko'

# --- File Source Configuration (only used if DATA_SOURCE_TYPE is 'file') ---
DATA_PATHS = {
    "DEFAULT_SAMPLE": os.path.join("data", "sample_btc_data.csv"),
}
CURRENT_DATA_KEY = "DEFAULT_SAMPLE"

# --- CoinGecko Source Configuration (only used if DATA_SOURCE_TYPE is 'coingecko') ---
CG_COIN_ID = 'bitcoin'  # e.g., 'bitcoin', 'ethereum', 'solana'
CG_VS_CURRENCY = 'usd' # e.g., 'usd', 'eur', 'gbp'
CG_DAYS = '365'         # Number of days of data (e.g., '7', '30', '365', 'max')
# --- End Configuration ---

# Initialize data at the global level
data = pd.DataFrame()

def load_data_from_file(path: str) -> pd.DataFrame:
    """Loads and prepares data from a local CSV file."""
    absolute_path = path
    ...
    if not os.path.exists(absolute_path):
        raise FileNotFoundError(
            f"Error: Data file not found at {absolute_path}. "
            f"Please ensure the file exists or update DATA_PATHS in the script. "
            f"If using a relative path, it should be relative to {SCRIPT_DIR}."
        )
    
    print(f"Loading data from file: {absolute_path}")
    df = pd.read_csv(absolute_path, parse_dates=['datetime'], index_col='datetime')
    df.columns = ['Open', 'High', 'Low', 'Close', 'Volume']
    df = df.sort_index(ascending=True)
    return df

def fetch_coingecko_ohlcv(coin_id: str, vs_currency: str, days: str) -> pd.DataFrame:
    """Fetches OHLCV data from CoinGecko API and returns a DataFrame."""
    cg = CoinGeckoAPI()
    print(f"Fetching {days} days of OHLCV data for {coin_id}/{vs_currency} from CoinGecko...")
    
    # CoinGecko API has rate limits (typically 10-30 calls per minute on free plan)
    # Be mindful if making many calls in a short period.
    try:
        ohlc_data = cg.get_coin_ohlc_by_id(id=coin_id, vs_currency=vs_currency, days=days)
    except Exception as e:
        print(f"Error fetching data from CoinGecko: {e}")
        print("This might be due to rate limiting or an invalid coin_id/vs_currency.")
        print("Please check CoinGecko for valid IDs: https://www.coingecko.com/en/api/documentation")
        return pd.DataFrame() # Return empty DataFrame on error

    if not ohlc_data:
        print(f"No OHLC data returned from CoinGecko for {coin_id} / {vs_currency} for {days} days.")
        return pd.DataFrame()
    ...
    # CoinGecko's OHLC endpoint doesn't directly provide volume for each candle.
    # We can fetch market_chart data which includes volume, then merge or align.
    # For simplicity here, we'll add a dummy Volume column if it's missing,
    # as your strategy doesn't seem to use Volume directly from data.df.Volume.
    # If your strategy later needs accurate volume, this part needs refinement.
    if 'Volume' not in df.columns:
        print("Warning: CoinGecko OHLC endpoint does not provide volume. Adding a dummy Volume column (all zeros).")
        print("If your strategy requires actual volume, consider using 'get_coin_market_chart_by_id' and processing its output.")
        df['Volume'] = 0.0 
        
    df = df.astype({'Open': float, 'High': float, 'Low': float, 'Close': float, 'Volume': float})
    df = df.sort_index(ascending=True)
    print(f"Successfully fetched and processed data for {coin_id}/{vs_currency}.")
    return df
...
# data.columns = ['Open', 'High', 'Low', 'Close', 'Volume'] # Moved to load_data

# Sort index in ascending order to fix the warning
# data = data.sort_index(ascending=True) # Moved to load_data
...
def run_and_display_backtest(
    ...
    **kwargs 
    ...
    """Runs a backtest, prints stats, and plots the results."""
    bt = Backtest(data_df, strategy_class, cash=cash, commission=commission)
    ...
    print(f"\n📊 Showing plot for {title} (close plot to continue)...")
    bt.plot(filename=plot_filename)
    # plt.show() # Backtesting.plot() now handles showing the plot if filename is None
    return stats, bt

# Create and run the backtest with initial parameters
# bt = Backtest(data, MAReversalStrategy, cash=1000000, commission=0.002) # Replaced by function call
...
if __name__ == "__main__":
    ...
    if DATA_SOURCE_TYPE == 'file':
        if CURRENT_DATA_KEY not in DATA_PATHS:
            raise KeyError(f"Error: CURRENT_DATA_KEY '{CURRENT_DATA_KEY}' not found in DATA_PATHS. Available keys: {list(DATA_PATHS.keys())}")
        selected_data_path = DATA_PATHS[CURRENT_DATA_KEY]
        try:
            data = load_data_from_file(selected_data_path)
        except FileNotFoundError as e:
            print(e)
            print("Please create the necessary data file and try again, or update the script configuration.")
            exit(1)
    elif DATA_SOURCE_TYPE == 'coingecko':
        data = fetch_coingecko_ohlcv(coin_id=CG_COIN_ID, vs_currency=CG_VS_CURRENCY, days=CG_DAYS)
        if data.empty:
            print("Failed to fetch data from CoinGecko. Exiting.")
            exit(1)
    else:
        print(f"Error: Invalid DATA_SOURCE_TYPE: '{DATA_SOURCE_TYPE}'. Choose 'file' or 'coingecko'.")
        exit(1)
    ...
    # --- MOVED: All execution now happens ONLY here ---
    initial_stats, initial_bt = run_and_display_backtest(
        data_df=data,
        strategy_class=MAReversalStrategy,
        cash=1000000,
        commission=0.002,
        title="Moon Dev's Initial"
    )

    print("\n🚀 Moon Dev's Strategy Optimization Starting 🚀")
    print("=" * 50)

    optimization_results = initial_bt.optimize(
        ma_fast=range(10, 30, 3),
        ma_slow=range(30, 60, 3),
        take_profit=[i/100 for i in range(2, 20, 2)],
        stop_loss=[i/100 for i in range(2, 20, 2)],
    ...
    optimized_stats, optimized_bt = run_and_display_backtest(
        data_df=data,
        strategy_class=MAReversalStrategy,
        cash=1000000,
        commission=0.002,
        title="Moon Dev's Optimized",
