...
def open_order_deluxe(symbol_info, size):
    ...
    stop_result = exchange.create_stop_market_order(symbol, stop_side, size, stop_loss, params=stop_params)
    logger.info(f'Stop loss order result for {symbol}: {stop_result}')
    tp_params = {'reduceOnly': True}
    tp_side = 'sell' if is_buy else 'buy'
    tp_result = exchange.create_limit_order(symbol, tp_side, size, take_profit, params=tp_params)
...
class MarketAnalyzer:
    ...
    def should_quote_orders(self):
        ...
        df = self.get_market_data('BTC:USDT', '1m', 1/6)  # ~4 hours
        if df.empty:
            return False
...
def get_liquidations():
    exchange = create_exchange()
    liqs = exchange.fetch_liquidations('BTC:USDT')
