#!/usr/bin/env python3
"""
Trains and evaluates various ML models to predict the next period's closing price
of BTC based on technical indicators, and analyzes feature importance.
"""
...
CONFIG = {
    ...
    "IMPORTANCE_FILENAME": "feature_importance.csv", # Note: Wide format

    # --- Data & Features ---
    "TARGET_COLUMN": "close",
    "TARGET_SHIFT": -1, # Predict next period's close
    # Technical Indicators (add more as needed)
    ...
    "MODELS": {
        ...
        'RandomForestRegressor': RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1), # Use all cores
    ...
    # --- Feature Importance ---
    "CALC_PERMUTATION_IMPORTANCE": False, # Set True for SVR/MLP if included, can be slow
    "PERMUTATION_N_REPEATS": 5, # Lower repeats for speed if enabled

    # --- Plotting ---
    "PLOT_BEST_PREDICTIONS": True,
...
def main(config: Dict) -> None:
    ...
    output_dir = os.path.join(config["RESULTS_DIR_PATH"])
    os.makedirs(output_dir, exist_ok=True)
    ...
    df, features = calculate_indicators(df, config)
    if not features:
    ...
    try:
        X, y = prepare_features_target(df, features, config["TARGET_COLUMN"], config["TARGET_SHIFT"])
    ...
    for name, model_instance in tqdm(models.items(), desc="Training Models"):
        try:
            metrics, importance, y_pred = train_evaluate_model(
                name, model_instance, X_train, y_train, X_test, y_test, features, config
            )
    ...
    perf_filepath = os.path.join(output_dir, config["PERFORMANCE_FILENAME"])
    ...
    if valid_importances:
        ...
        imp_filepath = os.path.join(output_dir, config["IMPORTANCE_FILENAME"])
    ...
    if not perf_df.empty and perf_df['R2'].notna().any():
        ...
        if config.get("PLOT_BEST_PREDICTIONS", False) and best_model_name in predictions:
            plot_actual_vs_predicted(
                y_test, predictions[best_model_name], best_model_name,
                output_dir, limit=config.get("PLOT_SAMPLE_SIZE")
            )

        # Plot feature importances
        if config.get("PLOT_FEATURE_IMPORTANCE", False):
            for name, importance_series in all_importances.items():
                 plot_feature_importance(importance_series, name, output_dir)
    else:
    ...
    except: pass # Avoid error if perf_df is empty
