...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.StreamHandler(), logging.FileHandler(f'bitfinex_vwap_trading_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')])
logger = logging.getLogger('bitfinex_vwap_trader')

class VWAPTrader:
    def __init__(self, symbol='BTC:USTF0', timeframe='15m', limit=100, sma_period=20, target_profit=9, max_loss=-8, max_risk=1000):
        self.symbol = symbol
        ...
        self.order_params = {'timeInForce': 'POC'}
        self.position_indices = {
            'BTC:USTF0': 0,  # Adjust indices based on Bitfinex symbols
    ...
    def check_pnl(self):
        ...
        current = ticker['last']
        entry = pos_info[0][0]['entry_price']
