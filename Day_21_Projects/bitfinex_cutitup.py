...
symbols = ['BTC:USDT', 'ETH:USDT', 'SOL:USDT', 'WIF:USDT', '1000PEPE:USDT']
...
def fetch_liquidations(symbol, since):
    ...
    for liq in liqs:
        ...
        data.append({
            ...
            'time_in_force': info.get('tif'),
            'original_quantity': float(info.get('qty', 0)),
            'price': float(info.get('px', 0)),
            'average_price': float(info.get('avgPx', 0)),
            'order_status': info.get('status'),
            'order_last_filled_quantity': float(info.get('lastQty', 0)),
            'order_filled_accumulated_quantity': float(info.get('cumQty', 0)),
            'order_trade_time': int(info.get('time', 0)),
            'usd_size': float(info.get('cumQty', 0)) * float(info.get('avgPx', 0))
        })
