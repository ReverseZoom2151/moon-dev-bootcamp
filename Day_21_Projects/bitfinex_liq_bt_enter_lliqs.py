...
l_liq_data = data[data['LIQ_SIDE'] == 'L LIQ'].copy()
ohlc_agg = {'price': 'mean', 'usd_size': 'sum'}
data_resampled = l_liq_data.resample('T').agg(ohlc_agg)
...
data_resampled.rename(columns={'usd_size': 'l_liq_volume'}, inplace=True)
price_columns = ['price', 'Open', 'High', 'Low', 'Close']
data_resampled[price_columns] = data_resampled[price_columns].ffill()
data_resampled['l_liq_volume'].fillna(0, inplace=True)
