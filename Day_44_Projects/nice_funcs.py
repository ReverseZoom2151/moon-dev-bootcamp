...
from Day_4_Projects.dontshare import key as xP_KEY # Use the key from dontshare.py
...
def get_sz_px_decimals(symbol):
    ...
    """
    sz_decimals = 0  # Default value
    px_decimals = 0  # Default value
    ask_price_for_print = "N/A"
    ...
    try:
        ...
        if symbol_info_item:
            ...
            print(f"Warning: Symbol '{symbol}' not found in metadata for sz_decimals. Using default sz_decimals={sz_decimals}.")

    except requests.exceptions.RequestException as e:
        print(f"Error fetching metadata for {symbol} to get sz_decimals: {e}. Using default sz_decimals={sz_decimals}.")
    except Exception as e: # Catch other potential errors like JSONDecodeError
        print(f"An unexpected error occurred while fetching sz_decimals for {symbol}: {e}. Using default sz_decimals={sz_decimals}.")

    try:
        # Fetch ask price for px_decimals calculation
        ask, _, _ = ask_bid(symbol) # We only need the ask price here
        ask_price_for_print = ask
        ask_str = str(ask)
        if '.' in ask_str:
            px_decimals = len(ask_str.split('.')[1])
        else:
            px_decimals = 0 # For whole numbers
    except Exception as e:
        print(f"Error fetching ask price or calculating px_decimals for {symbol}: {e}. Using default px_decimals={px_decimals}.")
        # px_decimals remains its default (0)

    print(f"For symbol '{symbol}': Using sz_decimals={sz_decimals}, px_decimals={px_decimals}. (Ask price for context: {ask_price_for_print})")
...
def limit_order(coin, is_buy, sz, limit_px, reduce_only, account):
    ...
    print(f"🔍 Raw order result (type {type(order_result)}): {order_result}")
    ...
    if isinstance(order_result, dict) and 'response' in order_result:
        ...
        print(f"✅ Order placed (raw response)")
...
def adjust_leverage(symbol, leverage):
    account = LocalAccount = eth_account.Account.from_key(xP_KEY)
    exchange = Exchange(account, constants.MAINNET_API_URL)
    info = Info(constants.MAINNET_API_URL, skip_ws=True)
...
def kill_switch_mkt(symbol, account):
    ...
    🌙 Moon Dev's turbocharged kill switch 🚀
    """
    positions, im_in_pos, pos_size, pos_sym, entry_px, pnl_perc, long = get_position(symbol, account)
    ...
    while im_in_pos == True:
        ...
        try:
            ...
            print(f"🎯 Using 5th bid: {fifth_bid} and 5th ask: {fifth_ask}")
        except (IndexError, KeyError):
            print("⚠️ Couldn't get 5th bid/ask, using best bid/ask instead")
        ...
        if long == True:
            ...
            print('🌙 Kill switch MKT - SELL TO CLOSE SUBMITTED at 5th bid')
            time.sleep(5)
        elif long == False:
            limit_order(pos_sym, True, pos_size, fifth_ask, True, account)
            print('🌙 Kill switch MKT - BUY TO CLOSE SUBMITTED at 5th ask')
            time.sleep(5)
...
def get_current_price(symbol):
    ...
    return (ask + bid) / 2  # Return the mid price as the current price

def get_balance(account):
    """
    Get the current account balance from Hyperliquid
    ...
    try:
        ...
        exchange = Exchange(constants.MAINNET_API_URL)
        
        # Get user state which includes balance info
        user_state = info.user_state(account.address)
        
        # Check if we got proper data
        if user_state and 'marginSummary' in user_state and 'accountValue' in user_state['marginSummary']:
            # Get account value directly from marginSummary
            account_value = float(user_state['marginSummary']['accountValue'])
            print(f"🌙 MOON DEV BALANCE CHECK: ${account_value:,.2f} 💰")
            return account_value
            
        # If marginSummary not available, try to calculate from positions
        elif user_state and 'assetPositions' in user_state:
            balance = 0.0
            
            # Sum up all positions value
            for asset in user_state['assetPositions']:
                if 'position' in asset and 'szi' in asset['position'] and 'entryPx' in asset['position']:
                    try:
                        pos_size = float(asset['position']['szi'])
                        entry_px = float(asset['position']['entryPx'])
                        balance += abs(pos_size * entry_px)
                    except (ValueError, TypeError):
                        continue
            
            if balance > 0:
                print(f"🌙 MOON DEV BALANCE CHECK (from positions): ${balance:,.2f} 💰")
                return balance
            
        # Try direct account value API as fallback
        try:
            account_value = exchange.get_account_value(account.address, account)
            if 'data' in account_value and 'accountValue' in account_value['data']:
                balance = float(account_value['data']['accountValue'])
                print(f"🌙 MOON DEV BALANCE CHECK (fallback): ${balance:,.2f} 💰")
                return balance
        except:
            pass
                
        print(f"❌ Failed to get balance: Unexpected response format")
        return 0.0
...
def get_account_value(account):
    """
    Get account information including total value
    ...
    """
    try:
        # Connect to Hyperliquid using the existing pattern
        info = Info(constants.MAINNET_API_URL)
        exchange = Exchange(constants.MAINNET_API_URL)
        
        # Get account value using your existing patterns
        response = exchange.get_account_value(account.address, account)
        return response
    except Exception as e:
...
def connect():
    ...
    Returns:
        dict: Connection with exchange and wallet
    """
    try:
        print("🌙 MOON DEV connecting to Hyperliquid API... 🚀")
        # Connect to Hyperliquid using the same pattern as in other functions
        info = Info(constants.MAINNET_API_URL)
        exchange = Exchange(constants.MAINNET_API_URL)
