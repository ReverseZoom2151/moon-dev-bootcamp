...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler("binance_trading_bot.log"), logging.StreamHandler()])
...
def load_config(config_file='binance_config.ini'):
    try:
        ...
        return {'symbol': 'BTCUSDT', 'pos_size': 30, 'target': 8, 'max_loss': -9, 'vol_decimal': 0.4}
...
def create_default_config():
    if not os.path.exists('binance_config.ini'):
        config = configparser.ConfigParser()
        config['Trading'] = {'symbol': 'BTCUSDT', 'position_size': '30', 'target': '8', 'max_loss': '-9', 'vol_decimal': '0.4'}
        with open('binance_config.ini', 'w') as configfile:
            config.write(configfile)
        logger.info("Created default binance_config.ini")
