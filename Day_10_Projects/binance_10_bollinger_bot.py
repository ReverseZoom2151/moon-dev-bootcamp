...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.StreamHandler(), logging.FileHandler(f'binance_bollinger_bot_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')])
logger = logging.getLogger('binance_bollinger_bot')
...
class BollingerBot:
    ...
    def check_position_status(self):
        try:
            ...
            im_in_pos = len(positions) > 0 and positions[0]['contracts'] != 0
            pos_size = positions[0]['contracts'] if im_in_pos else 0
            long = positions[0]['side'] == 'long' if im_in_pos else None
            entry_px = positions[0]['entryPrice'] if im_in_pos else 0
            pnl_perc = positions[0]['unrealizedPnl'] / (entry_px * pos_size) * 100 if im_in_pos and entry_px != 0 else 0
            num_of_pos = len([p for p in self.exchange.fetch_positions() if p['contracts'] != 0])
    ...
    def check_bollinger_bands(self):
        try:
            bars = self.exchange.fetch_ohlcv('BTCUSDT', '1m', limit=500)  # Using BTC as proxy
            ...
            bands_tight = bandwidth.iloc[-1] < 0.05
