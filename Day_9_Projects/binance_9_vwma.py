...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler("binance_vwma_trading.log"), logging.StreamHandler()])
logger = logging.getLogger("BinanceVWMATrader")
...
def load_config(config_file='binance_vwma_config.ini'):
    try:
        ...
        return {'symbol': 'BTCUSDT', 'size': 1, 'target': 9, 'max_loss': -8, 'timeframe': '1d', 'limit': 100}
...
class VWMATrader:
    def __init__(self, exchange, config=None):
        ...
        self.position_indices = {'BTCUSDT': 0}  # Adjust as needed
...
def create_default_config():
    if not os.path.exists('binance_vwma_config.ini'):
        config = configparser.ConfigParser()
        config['Trading'] = {'symbol': 'BTCUSDT', 'size': '1', 'target': '9', 'max_loss': '-8'}
        config['Indicators'] = {'timeframe': '1d', 'limit': '100'}
        with open('binance_vwma_config.ini', 'w') as configfile:
            config.write(configfile)
        logger.info("Created default binance_vwma_config.ini")
