...
class Agent:
    ...
    def plan_tasks(self, query: str) -> List[Task]:
        # Handle both tool objects and functions decorated with @tool
        tool_descriptions = []
        for t in TOOLS:
            if hasattr(t, 'name') and hasattr(t, 'description'):
                # LangChain tool with metadata
                tool_descriptions.append(f"- {t.name}: {t.description}")
            elif hasattr(t, '__name__'):
                # Regular function or decorated function
                name = getattr(t, '__name__', 'unknown')
                # For @tool decorated functions, the description is in the docstring
                desc = getattr(t, '__doc__', 'No description available')
                if desc:
